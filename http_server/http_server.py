...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.wfile.write(('<tr><td>CPU</td><td>{}</td></tr>'.format(1)).encode('utf-8'))
        self.wfile.write(('<tr><td>Uso da CPU</td><td>{}</td></tr>'.format(1)).encode('utf-8'))
        ...
        self.wfile.write('</table>'.encode('utf-8'))
        self.wfile.write('<style>html{background-color:#e9eef2;color:#fff;font-family:"Roboto Mono",monospace;height:100%;width:100%}table{background-color:#78a6c8;border:8px solid rgba(0,0,0,0%);border-collapse:collapse;border-radius:8px}tr>*+*{padding-left:2em}hr{margin:8px 0}</style>'.encode('utf-8'))
