...
class MyHandler(BaseHTTPRequestHandler):
    def getprocesses(self):
        return subprocess.check_output(['ps', '-o', 'pid=,comm=']).decode('utf-8').split('\n')
    ...
    def do_GET(self):
        ...
        self.wfile.write('<table>'.encode('utf-8'))
        self.wfile.write('<thead><tr><th colspan="2">Processos</th></tr><tr><th colspan=2 align=left>PID / Nome</th></tr></thead>'.encode('utf-8'))
        for line in processes:
            self.wfile.write(('<tr><td colspan=2>{}</td></tr>'.format(line)).encode('utf-8'))
        self.wfile.write('</table>'.encode('utf-8'))
